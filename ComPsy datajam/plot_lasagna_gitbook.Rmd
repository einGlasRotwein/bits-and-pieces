...
---
# Plot Lasagna
Let's dive into the world of plots in `R`. If you have worked with images before, you might be familiar with the general logic of layers in graphics. That is, different components of an image are stacked on top of each other to create something awesome - like a lasagna. Importantly, certain components can be altered without changing the rest of them. For example, you could replace the tomato sauce in an lasagna with sauce hollandaise, but keep the pasta, the minced meat, the cheese etc. We'll get to that later and explore the `tidyverse` first, the family of packages that enables us to work with graphics in `R` in this layered manner. 
...
We briefly encountered the tidyverse in our short trip to `rmarkdown` in the .Rmd example of rendering a PDF document: The diamond data set we used lives in the `tidyverse`. The `tidyverse` is a package in `R`, but it is a bit of an odd one. See what it does when we load it (and let the chunk show the message for once).
...
No we finally get back to our analogy. Graphics work in layers. Like lasagna. Our first layer is the panel which we draw on. Which essentially means empty space, so we don't see anything when we pipe our data to the `ggplot` function.
