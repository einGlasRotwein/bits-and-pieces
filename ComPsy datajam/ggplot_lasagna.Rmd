...
diamonds_small <- diamonds %>%
  ...
  mutate(clarity = factor(clarity), cut = factor(cut))
```
...
Want to have these fancy little asterisks in your plots? Check out `ggpubr` (for a detailed blog post, go [here](https://www.r-bloggers.com/add-p-values-and-significance-levels-to-ggplots/))! The default is a Kruskal Wallis test for the overall effect and Wilcoxon Tests for the pairwise comparisons, but we can specify an ANOVA and a t.test (better double check with your results anyways). The pairwise comparisons are already Bonferroni corrected.

```{r}
library(ggpubr)
pairwise_comparisons <- list( c("Fair", "Very Good"), c("Very Good", "Ideal"), 
                              c("Fair", "Ideal") )
...
diamonds_small %>% 
  ...
  stat_summary(fun.y = mean,
               fun.ymax = function(x) mean(x) + sd(x),
               fun.ymin = function(x) mean(x) - sd(x)) +
  stat_compare_means(comparisons = pairwise_comparisons, method = "t.test") +
  stat_compare_means(label.y = 50, method = "anova")
```

Fancy something a bit more minimalistic?
...
diamonds_small %>% 
  ...
  stat_summary(fun.y = mean,
               ...
               fun.ymin = function(x) mean(x) - sd(x)) +
  stat_compare_means(comparisons = pairwise_comparisons, method = "t.test",
                     label =  "p.signif", label.y = c(10000, 11000, 12000))
```

You can also speficy reference groups, it works for grouped plots as well ... possibilities are endless!
